...
public class CombineMesh : MonoBehaviour 
{
    ...
    static void StartCombineMesh()
    {
        if (Selection.activeTransform)
        {
            ...
            mesh.uv3 = null;
            AssetDatabase.CreateAsset(mesh, "Assets/Scenes/CombineMesh/combine.asset");
            go.GetComponent<MeshFilter>().sharedMesh = mesh;
            go.GetComponent<MeshRenderer>().sharedMaterial = material;
            if (go)
            {
                PrefabUtility.SaveAsPrefabAssetAndConnect(go, Application.dataPath + "/Scenes/CombineMesh/combine.prefab", InteractionMode.AutomatedAction);
            }
        }
